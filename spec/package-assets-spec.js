...
describe("linter package assets", () => {
  it("ships keymaps and menus as JSON, not CSON", () => {
    expect(exists("keymaps/linter.json")).toBe(true);
    expect(exists("menus/linter.json")).toBe(true);
  ...
  it("uses the linter: command prefix in the keymap and menu", () => {
    const keymap = JSON.parse(read("keymaps/linter.json"));
    expect(keymap["atom-workspace"]["alt-l"]).toBe("linter:toggle-focus");

    const menu = JSON.parse(read("menus/linter.json"));
    const flat = JSON.stringify(menu);
    expect(flat).toContain("linter:lint");
    expect(flat).not.toContain("linter-bundle:");
  ...
  it("keeps the shared linter service contract intact", () => {
    const pkg = JSON.parse(read("package.json"));
    expect(pkg.providedServices["linter-indie"].versions["2.0.0"]).toBe("provideIndie");
    expect(pkg.providedServices["mcp-tools"].versions["1.0.0"]).toBe("provideMcpTools");
    expect(pkg.consumedServices["linter"].versions["2.0.0"]).toBe("consumeLinter");
