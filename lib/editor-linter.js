...
class EditorLinter {
  constructor(editor) {
    ...
    this.subscriptions.add(
      ...
      this.editor.onDidChangeGrammar(() => this.lint("grammar")),
  ...
  lint(trigger = "manual") {
    this.emitter.emit("should-lint", trigger);
  }
  ...
  dispose() {
    // The editor's own destroy, the registry's teardown and a `linter.editors`
    // registration being disposed can each land here; only the first counts.
    if (this.disposed) {
      return;
    }
    this.disposed = true;
