...
async function isPathIgnored(filePath, ignoredGlob, ignoredVCS) {
  ...
  if (ignoredVCS) {
    // `Directory` is no longer exported from the `lumine` module in Lumine
...
function normalizeMessages(
  ...
  for (let i = 0, { length } = messages; i < length; ++i) {
    ...
    // paths by this value, and computing it per message per publish — in the
    // buffer assignment, in the panel's cursor scan, in the intentions and MCP
    // filters — was the largest single cost in the update path.
