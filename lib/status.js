class StatusPanel {
  constructor(pkg) {
    ...
    this.configDisposable = atom.config.observe("linter.statusMode", (value) => {
    ...
    this.tooltipDisposable = atom.tooltips.add(this.element, {
      title:
        '<div style="text-align: left; line-height: 1.2em;">Click to toggle panel<br />Middle-click to toggle file/project view<br />Ctrl+middle-click to clear all<br />Right-click to go next<br />Ctrl+right-click to go previous<br />Use linter:toggle-current-file to enable or disable linting for this file</div>',
      html: true,
    });
  ...
  onmouseup(e) {
    if (e.which === 2 && e.ctrlKey) {
      // ctrl+middle click
      atom.commands.dispatch(atom.views.getView(atom.workspace), "linter:clear");
