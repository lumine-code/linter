...
class Linter {
  ...
  /**
   * Whether an editor is one of the documents open in the workspace centre.
   *
   * A package builds editors of its own to render things with: a diff, a patch
   * preview, a commit box in a dock, the field inside a picker. None of those is
   * a document, and a provider that only means documents says so with
   * `editors: "center"`. A buffer nobody has saved yet is still one — it is a
   * pane item, it just has no path.
   *
   * An item that is not itself an editor reaches a provider through the editor an
   * adapter names for it, so a notebook counts through that editor.
   * @param {Object} editor
   * @returns {boolean}
   */
  isCenterEditor(editor) {
    for (const item of lumine.workspace.getCenter().getPaneItems()) {
      if (this.getTextEditorForItem(item) === editor) {
        return true;
      }
    }
    return false;
    ...
    this.registryEditors.observe((editorLinter) => {
      ...
      editorLinter.onShouldLint((onChange) => {
        ...
        this.registryLinters.lint({
          onChange,
          editor,
          // Answered lazily and at most once per run: only a provider that
          // declared `editors: "center"` makes anyone ask.
          isCenterEditor: () => this.isCenterEditor(editor),
        });
      });
    ...
    this.registryIndie.onDidUpdate(({ linter, messages, options }) => {
      if (linter.deleteOnOpen) {
        for (const editor of lumine.workspace.getTextEditors()) {
          // The delegate keys its map the same way, so an editor path finds
          // what a provider stored under another spelling of the same file.
