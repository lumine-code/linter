...
const { showToggleView } = require("./toggle-view");
...
class Linter {
  constructor() {
    ...
    this.commands.onShouldToggleLinter(() => {
      this.registryLintersInit();
      // Tearing the package down must take the list with it: its handlers write
      // config and lint through registries that are about to go. "destroyed" is
      // the reason that tells the view not to lint on the way out.
      let closeWithPackage = null;
      let closed = false;
      const session = showToggleView({
        providers: arrayUnique(this.registryLinters.getProviders().map((linter) => linter.name)),
        onDisable: (name) => {
        ...
        onClose: () => {
          closed = true;
          if (closeWithPackage) {
            this.subscriptions.remove(closeWithPackage);
          }
        },
      });
      // Null when the command closed a list that was already up; already closed
      // when the view never got off the ground.
      if (!session || closed) {
        return;
      }
      closeWithPackage = new Disposable(() => session.cancel("destroyed"));
      this.subscriptions.add(closeWithPackage);
    });
