...
class Linter {
  constructor() {
    ...
    this.commands.onShouldToggleLinter(() => {
      ...
      // Null when the command closed a list that was already up.
      if (!session) {
        return;
      }
      ...
      const closeWithPackage = new Disposable(() => session.cancel("destroyed"));
      this.subscriptions.add(closeWithPackage);
      session.onDidClose(() => this.subscriptions.remove(closeWithPackage));
    });
