  ...
  constructor() {
    ...
    this.subscriptions.add(
      ...
      lumine.commands.add("lumine-workspace", {
        "linter:lint": {
          description: "Run every registered linter over the active file now.",
          didDispatch: () => this.lint(),
        },
        "linter:debug": {
          description: "Report which linters are registered and how they are set up.",
          didDispatch: () => this.debug(),
        },
        // Two names for the one action, so they carry the one sentence.
        "linter:state": {
          description: "Turn linting on or off for the active file alone.",
          didDispatch: () => this.toggleActiveEditor(),
        },
        "linter:toggle-current-file": {
          description: "Turn linting on or off for the active file alone.",
          didDispatch: () => this.toggleActiveEditor(),
        },
        "linter:toggle-linter": {
          description: "Choose which of the registered linters are allowed to run.",
          didDispatch: () => this.toggleLinter(),
        },
      }),
    );
  }
...
async function showDebug(standardLinters, indieLinters, textEditor) {
  ...
  const matchesIgnoreGlob = Helpers.matchesIgnoreGlob(textEditor.getPath(), ignoreGlob);
  lumine.notifications.addInfo("Linter Debug Info", {
    detail: [
      `Platform: ${process.platform}`,
      `Lumine Version: ${lumine.application.getVersion()}`,
      `Linter Version: ${manifest.version}`,
      `Opened file matches Ignore Glob: ${matchesIgnoreGlob ? "Yes" : "No"}`,
