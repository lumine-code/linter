...
const { $version, $activated } = require("./helpers");
...
class LinterRegistry {
  ...
  beginRequest(linter, buffer) {
    const state = this.requests.get(linter);
    const number = ++state.nextNumber;
    if (buffer) {
      state.latestFileRuns.set(buffer, number);
    } else {
      state.latestProjectRun = number;
    }
    return number;
  }

  requestIsLatest(linter, buffer, number) {
    const state = this.requests.get(linter);
    if (!state) return false;
    return buffer ? state.latestFileRuns.get(buffer) === number : state.latestProjectRun === number;
  }

  clearFileResultsForGrammarChange(buffer) {
    for (const linter of this.linters) {
      if (linter.scope !== "file") continue;
      // A result already running belongs to the grammar that started it. Drop
      // its claim to be latest before clearing the published snapshot, so it
      // cannot put those messages back after the grammar has changed.
      this.requests.get(linter).latestFileRuns.delete(buffer);
      this.emitter.emit("did-update-messages", { linter, messages: [], buffer });
    }
  }

  async lint({ trigger = "manual", editor }) {
    const filePath = editor.getPath();
    const editorBuffer = editor.getBuffer();
    const onChange = trigger === "change";
    if (trigger === "grammar") {
      // Provider applicability is grammar-dependent. File results from the old
      // grammar stop being valid immediately, including results whose lint is
      // still in flight; matching providers repopulate them below.
      this.clearFileResultsForGrammarChange(editorBuffer);
    }
    if (
    ...
    for (const linter of this.linters) {
      ...
      const statusBuffer = linter.scope === "file" ? editorBuffer : null;
      const statusFilePath = linter.scope === "file" ? filePath : null;
      const number = this.beginRequest(linter, statusBuffer);
      ...
      promises.push(
        Promise.race([
          ...
          (messages) => {
            ...
            if (
              !linter[$activated] ||
              !this.requestIsLatest(linter, statusBuffer, number) ||
