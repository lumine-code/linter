...
class LinterUI {
  constructor() {
    ...
    // one is measured again. See isLargeBuffer.
  ...
  updateMarkers() {
    for (const buffer of this.getBuffers()) {
      ...
      if (wasLarge) {
        // It was too large and is not any more, so nothing it holds has been
        // decorated — all of it is new here.
      ...
      const currentKeys = new Set((state.messages || []).map((m) => m.key));
