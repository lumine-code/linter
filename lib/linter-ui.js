...
class LinterUI {
  ...
  assignMessages(args) {
    ...
    for (const message of args.added) {
      ...
      // Normalized: the provider and the buffer may spell the same file
      // differently, and this map is only ever read back by path.
      const path = normalizePath(message.location.file);
  ...
  revealMessage(message) {
    ...
    this.editor.scrollToCursorPosition({
      zone: atom.config.get("linter.editorScrollZone"),
    });
  ...
  showTooltipAtCursor() {
    const view = this.editor && atom.views.getView(this.editor);
    if (view) atom.commands.dispatch(view, "hover:toggle");
  }
