...
class LinterUI {
  ...
  getBuffers() {
    ...
    lumine.workspace.getTextEditors().forEach((editor) => buffers.add(editor.getBuffer()));
    return buffers;
  }
          ...
          if (invalidate === "touch") {
            ...
            marker.onDidChange(({ isValid }) => {
              if (!isValid && this.onDeleteMessage && markerMap.get(message.key)?.[0] === marker) {
                this.onDeleteMessage(this.markerMessageOrigins.get(message) || message);
                destroyMarkers(markerMap, message.key);
              }
            });
          }
