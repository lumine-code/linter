...
function destroyBufferState(buffer) {
  ...
  state.replaceSubscription?.dispose();
...
class LinterUI {
  ...
  /**
   * Delivers the standing message set to buffers patched after the last
   * render. Deferred one microtask so a batch of registrations — a notebook
   * building an editor per cell — resolves in a single pass, after the host
   * has finished wiring the editor it registered (an adapter cannot project
   * onto a cell editor its notebook has not indexed yet).
   * @param {TextBuffer} buffer - The freshly patched buffer
   */
  scheduleProjectionCatchUp(buffer) {
    this.catchUpBuffers = this.catchUpBuffers || new Set();
    this.catchUpBuffers.add(buffer);
    ...
    queueMicrotask(() => {
      ...
      const buffers = this.catchUpBuffers;
      this.catchUpBuffers = new Set();
      if (!this.allMessages.length) {
        return;
      }
      const markerMessages = this.getMarkerMessages(this.allMessages);
      let touched = false;
      for (const buffer of buffers) {
        const state = buffer.linterUI;
        if (!state || buffer.isDestroyed?.()) {
          continue;
        }
        const bufferPath = normalizePath(buffer.getPath());
        const mine = markerMessages.filter((message) => {
          if (message.location.buffer) {
            return message.location.buffer === buffer;
          }
          return message.location.normalizedFile === bufferPath;
        });
        if (!mine.length && !state.messages.length) {
          continue;
        }
        state.messages = mine.sort((a, b) =>
          a.location.position.start.compare(b.location.position.start),
        );
        touched = true;
      }
      // updateMarkers reconciles each touched buffer's markers against its
      // fresh message set; the untouched ones take the in-sync fast path.
      if (touched) {
        this.updateMarkers();
      }
    });
  ...
  patchEditor(editor) {
    ...
    if (!buffer.linterUI) {
      // The standing message set only reaches a buffer during a render, and a
      // render only runs when the registry reports a change — a buffer that
      // appears while the messages hold still (a notebook cell editor built
      // after its diagnostics arrived, or rebuilt around an undo) would wait
      // for the next publish and show nothing until then.
      this.scheduleProjectionCatchUp(buffer);
      buffer.linterUI = {
        severityLayers: {},
      ...
      // A whole-buffer replace — a notebook undo restoring a cell's source, a
      // reload pushing external content through setText — drags every tracked
      // marker to the splice edges, and the message set often comes back
      // byte-identical so no render would ever repair them. Positions from the
      // messages are the only truth left; rebuild from those.
      buffer.linterUI.replaceSubscription = buffer.onDidChangeText((event) => {
        const state = buffer.linterUI;
        if (!state?.markerMap.size || event.changes.length !== 1) {
          return;
        }
        ...
        const isFullReplace =
          ...
          change.newRange.end.isEqual(buffer.getEndPosition());
        if (!isFullReplace) {
          return;
        }
        for (const key of Array.from(state.markerMap.keys())) {
          destroyMarkers(state.markerMap, key);
        }
        for (const message of state.messages || []) {
          delete message.location.displayRange;
        }
        this.updateMarkers();
      });
    }
