...
class LinterUI {
  constructor() {
    ...
    this.subscriptions.add(
      ...
      atom.config.observe("linter.largeFileLineCount", (value) => {
        this.largeFileLineCount = value || DEFAULT_LARGE_FILE_LINE_COUNT;
      }),
      atom.config.observe("linter.longLineLength", (value) => {
  ...
  dispose() {
    ...
    for (const buffer of this.getBuffers()) {
      if (buffer.linterUI) {
        ...
        const layers = [
          ...Object.values(buffer.linterUI.severityLayers || {}),
          ...Object.values(buffer.linterUI.tagLayers || {}),
        ];
  ...
  patchEditor(editor) {
    ...
    for (const severity of SEVERITIES) {
      ...
      // a dot on from styles.less with no code change.
      editor.decorateMarkerLayer(layer, {
        type: "line-number",
    ...
    for (const buffer of this.getBuffers()) {
      ...
      if (addedMessages?.length) {
        for (const message of addedMessages) {
          ...
          // Index 0 is always the severity marker: it is the one whose
          // invalidation retires the message. Tag markers cover the same range
          // so their decorations coincide exactly and the merged span carries
          // both classes across the whole range.
          const markers = [
            severityLayer.markRange(message.location.displayRange, { invalidate: "touch" }),
          ];
          for (const tag of message.tags || []) {
            const tagLayer = buffer.linterUI.tagLayers[tag];
            if (tagLayer) {
              markers.push(
                tagLayer.markRange(message.location.displayRange, { invalidate: "touch" }),
              );
            }
          }
          markerMap.set(message.key, markers);
          // Only the severity marker is observed. Every marker of a message
          // shares one range and one invalidation strategy, so they collapse in
          // the same transaction and a listener per tag would only fire twice.
          const marker = markers[0];
          marker.onDidChange(({ isValid }) => {
            // Guard: only act if this marker is still the tracked one for this key.
            if (!isValid && this.onDeleteMessage && markerMap.get(message.key)?.[0] === marker) {
  ...
  revealMessage(message) {
    ...
    scrollToCursorWithMode(this.editor, atom.config.get("linter.editorScrollPosition"));
