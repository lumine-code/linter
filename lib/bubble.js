...
class BubblePanel {
  ...
  showTooltipForMessages(messages, event, editor) {
    ...
    for (const message of messages) {
      ...
      content.innerHTML = atom.tools.markdown.render(message.excerpt);

      // The long form below the excerpt, as plain text: it carries metadata such
      // as a rule code and related locations, which markdown would reflow. A
      // hover is the "expanded" gesture for a lazy description, so it is
      // resolved here and lands in the element if this tooltip is still up.
