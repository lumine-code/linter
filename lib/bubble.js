...
const { getDescription, resolveDescription, scrollToCursorWithMode } = require("./helpers");
...
class BubblePanel {
  ...
  showTooltipForMessages(messages, event, editor) {
    ...
    for (const message of messages) {
      ...
      excerpt.classList.add("linter-bubble-excerpt");
      excerpt.innerHTML = atom.tools.markdown.render(message.excerpt);
      content.appendChild(excerpt);

      // The long form trails the excerpt on the same line, as plain text: it
      // carries metadata such as a rule code and related locations, which
      // markdown would reflow. Always in the DOM — empty until it has text, so
      // a lazy description lands beside the excerpt rather than after whatever
      // was appended while it resolved. A hover is the "expanded" gesture for
      // such a description, so it is resolved here and fills the element if
      // this tooltip is still up.
      const detail = document.createElement("div");
      detail.classList.add("linter-bubble-detail");
      ...
      if (description) {
        ...
        resolveDescription(message).then((text) => {
          if (!text || this.tooltip !== tooltip) return;
