      ...
      inputSchema: {
        ...
        properties: {
          filePath: {
            type: "string",
            description:
              "Absolute path of the file to return messages for. Works even when the file is not open in a tab. Path separators are normalized, so '/' and '\\' are treated as equal.",
          },
          severity: {
            type: "string",
...
/**
 * Normalize a file path for comparison: unify separators and case-fold, so
 * '/' vs '\\' and drive-letter casing differences do not cause mismatches.
