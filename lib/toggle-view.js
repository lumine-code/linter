...
function showToggleView({ providers, onDisable, onFinish, onClose }) {
  let changed = false;
  ...
  return atom.modals.toggle({
    ...
    // This runs for the list's own frame under every close reason, which is the
    // only signal the caller can rely on: dispatched from inside another
    // modal's action — which is what the command palette does — the list is
    // pushed as a sublist, and popping it leaves the session that `toggle()`
    // returned very much alive. Bookkeeping first, so a throwing `onFinish`
    // cannot strand it.
    didClose: (result) => {
      onClose?.(result);
      // Linting is deferred to the close so a run of toggles costs one lint
      // pass, not one per provider. A view that is going away with the window
      // or the package has nothing left to lint through.
